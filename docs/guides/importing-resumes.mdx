...
description: "Import a resume from JSON, JSON Resume, PDF, or Microsoft Word files with automatic format detection from the TNM HR Platform dashboard."
---

TNM HR Platform can create a new resume from several existing file formats. Use import when you are moving from another tool, restoring a backup, or converting an older TNM HR Platform file.

## Supported import formats

| Format | Requires AI integration? | Notes |
| --- | --- | --- |
| **TNM HR Platform (JSON)** | No | Best option for backups exported from the current version of TNM HR Platform. |
| **TNM HR Platform v4 (JSON)** | No | Use this for files exported from TNM HR Platform v4. |
| **JSON Resume** | No | Use this for files that follow the JSON Resume schema. |
| **PDF** | Yes | TNM HR Platform asks your configured AI provider to parse the file into structured resume data. |
| **Microsoft Word** | Yes | Supports Word documents. TNM HR Platform asks your configured AI provider to parse the document. |
...
<Steps>
  ...
  <Step title="Choose your file">
    Select the resume file from your computer. TNM HR Platform detects the format automatically from the file's contents
  ...
  <Step title="Import">
    Click **Import**. TNM HR Platform creates a new resume and opens it in the builder when the import succeeds.
...
If you have a file exported from TNM HR Platform, choose **TNM HR Platform (JSON)**. This preserves the most information because the file already matches TNM HR Platform's data model.

If you are coming from TNM HR Platform v4, choose **TNM HR Platform v4 (JSON)**.
...
Make sure you selected the correct JSON import type. A TNM HR Platform JSON export, a TNM HR Platform v4 JSON export, and a JSON Resume file are different formats.
