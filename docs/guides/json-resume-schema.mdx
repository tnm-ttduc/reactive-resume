...
description: "Learn about the TNM HR Platform JSON Schema, available at /schema.json, and how it aims to become the gold standard for portable resume data."
---
...
TNM HR Platform aims to be the **gold standard in JSON Resume Schemas**. A universal, well-documented, and rigorously typed schema that any service can adopt to enable seamless resume portability across the entire job application ecosystem.

Imagine:

- Exporting your resume from TNM HR Platform as a JSON file
...
The TNM HR Platform JSON Schema is publicly available at:

<Info>[http://localhost:3000/schema.json](http://localhost:3000/schema.json)</Info>

This endpoint returns a complete JSON Schema (draft 07 compatible) that describes the structure of a TNM HR Platform document. It includes:
...
When importing resumes, TNM HR Platform uses this version to handle migrations and ensure backward compatibility.
...
const ajv = new Ajv();
const schema = await fetch("http://localhost:3000/schema.json").then((r) => r.json());
...
- **Job portals** can accept TNM HR Platform JSON files directly
...
Below is the complete JSON Schema for TNM HR Platform. You can also fetch the latest version directly from [http://localhost:3000/schema.json](http://localhost:3000/schema.json).
