...
description: "Connect TNM HR Platform to AI tools like Claude Desktop, Cursor, and Codex through the Model Context Protocol to edit and manage resumes via chat."
---

The TNM HR Platform MCP server lets you manage and modify your resumes and job applications through any MCP-compatible AI tool - Claude Desktop, Cursor, Codex, and more. It connects to the TNM HR Platform API and exposes tools for resume editing, Application Tracker workflows, and AI-assisted job application tasks using natural language.
...
<Steps>
  <Step title="Choose your authentication method">
    TNM HR Platform MCP supports two authentication methods:
  ...
  <Step title="If using API key, create one">
    Head over to [http://localhost:3000](http://localhost:3000) (or your self-hosted instance), sign in, and navigate to **Settings → API Keys**. Click **Create a new API key**, give it a name, and copy the secret — it's only shown once.
...
```json
{
  "mcpServers": {
    "tnm-hr-platform": {
      "command": "npx",
      "args": ["mcp-remote", "http://localhost:3000/mcp", "--header", "x-api-key:your-api-key"]
    }
  }
}
```
...
## Authentication Details (How TNM HR Platform MCP Works)

TNM HR Platform MCP accepts authentication in this order:
...
TNM HR Platform is configured as an OAuth authorization server for MCP clients:
...
```bash
codex mcp add tnm-hr-platform --url http://localhost:3000/mcp
```

Then log in with OAuth:

```bash
codex mcp login tnm-hr-platform
```

API key fallback (`config.toml`):

```toml
[mcp_servers."tnm-hr-platform"]
url = "http://localhost:3000/mcp"
...
Add `http://localhost:3000/mcp` as a custom remote MCP connector, then connect with OAuth in Claude's connector UI.
...
If you're running a self-hosted TNM HR Platform instance, replace `http://localhost:3000/mcp` with your instance URL:
...
| "API error (403)"                                             | The resume is locked. Unlock it in the TNM HR Platform dashboard                                                               |
