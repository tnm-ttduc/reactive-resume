...
description: "Partially update a TNM HR Platform with JSON Patch (RFC 6902) operations to add, remove, replace, and move fields without sending the full document."
---
...
<Info>
  If you're self-hosting, replace `http://localhost:3000` with your instance URL. The API is served under `/api/openapi`.
</Info>
...
```bash
curl -X PATCH "http://localhost:3000/api/openapi/resume/YOUR_RESUME_ID" \
