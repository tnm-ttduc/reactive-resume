...
description: "A comprehensive guide to self-host TNM HR Platform with Docker (Postgres only), including a detailed environment variable reference and troubleshooting tips."
---
...
TNM HR Platform can be self-hosted using Docker in a matter of minutes, and this guide will walk you through the process. Here are some of the services you'll need to get started:
...
- Docker Hub: `tnm-hr-platform:latest`
- GitHub Container Registry: `tnm-hr-platform:latest`
...
Create a new folder (for example `tnm-hr-platform/`) with:
...
SMTP_USER=""
SMTP_PASS=""
SMTP_FROM="TNM HR Platform <noreply@localhost:3000>"
SMTP_SECURE="false"
...
# The rest of TNM HR Platform can run without these.
REDIS_URL=""
# Generated using `openssl rand -hex 32`
ENCRYPTION_SECRET=""
...
```
  ...
  <Step title="Create compose.yml">
    This setup runs Postgres and TNM HR Platform on a private Docker network.
...
```
    ...
    <Tip>
      Prefer pulling from Docker Hub? Keep <code>tnm-hr-platform:latest</code>. Prefer GHCR? Swap it to <code>tnm-hr-platform:latest</code>.
    </Tip>

    <Note>
      In Docker, the TNM HR Platform server listens on <code>PORT</code> and serves both the API and the built web app.
...
```bash
docker compose logs -f tnm-hr-platform
```

    </CodeGroup>

    TNM HR Platform should now be available at your `APP_URL` (for the example above: `http://localhost:3000`).
...
```
  ...
  <Accordion title="Email (SMTP, optional)">
    ...
    - **`SMTP_FROM`**: Default from address (for example, `TNM HR Platform <noreply@localhost:3000>`).
...
<Accordion title="AI features (optional)">
  Saved AI provider management is usable only when **`ENCRYPTION_SECRET`** is configured. The AI Agent workspace also requires **`REDIS_URL`**. The rest of TNM HR Platform can run without them.
...
To update your TNM HR Platform installation to the latest available version, follow these steps:
...
Regular backups are essential to protect your data. TNM HR Platform stores data in two places: the PostgreSQL database and file uploads (either local storage or S3).
...
TNM HR Platform exposes a health check endpoint at `/api/health` that verifies the application and its dependencies. It checks **database** and **storage**; if either is unhealthy, the endpoint returns HTTP `503`.
...
This is particularly useful in high-availability setups where you have multiple instances of TNM HR Platform. If one instance becomes unhealthy (for example, it loses database or storage connectivity), the reverse proxy will stop routing traffic to it until it recovers.
...
You can manually verify the health of your TNM HR Platform instance:
