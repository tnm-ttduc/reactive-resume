...
description: "Step-by-step guide to migrate a self-hosted TNM HR Platform instance from v4 to v5, covering database schema changes, manual and automated options."
---

## Overview

This guide walks you through migrating your TNM HR Platform installation from **v4 to v5**. The migration process involves setting up a new v5 instance alongside your existing v4 instance, then transferring your users and resumes to the new system.
...
<CardGroup cols={2}>
  <Card title="Running v4 Instance">Your existing TNM HR Platform v4 instance should be running and accessible.</Card>
  <Card title="New v5 Instance">
    A fresh TNM HR Platform v5 instance set up and running. Follow the [Self-Hosting with Docker](/self-hosting/docker)
...
<Step title="Import into v5">
  ...
  Select the **TNM HR Platform v4** format 4. Upload your exported JSON file The import process automatically converts
  the v4 format to v5.
</Step>
...
<CardGroup cols={1}>
  ...
  <Card title="TNM HR Platform Source Code">
    Clone the TNM HR Platform repository and check out the last tag that includes the migration scripts:
    ```bash
