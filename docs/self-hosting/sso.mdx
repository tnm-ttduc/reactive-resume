...
description: "Configure Single Sign-On for self-hosted TNM HR Platform using custom OAuth providers like Authentik, Authelia, Keycloak, or any OIDC-compliant IdP."
---

## Overview

TNM HR Platform supports custom OAuth providers, allowing you to integrate with enterprise identity providers and self-hosted authentication solutions. This is particularly useful for organizations that want to:
...
<Info>
  ...
  [localhost:3000](http://localhost:3000), you can use the built-in Google and GitHub sign-in options.
</Info>
...
<Tabs>
  <Tab title="Option A: OIDC Discovery (Recommended)">
    ...
    **Examples:**
    - Authentik: `https://auth.example.com/application/o/tnm-hr-platform/.well-known/openid-configuration`
...
TNM HR Platform automatically maps user profile data from the OAuth provider. The following fields are used:

| TNM HR Platform Field | OAuth Profile Fields (in order of preference) |
| --------------------- | --------------------------------------------- |
...
OAUTH_DISCOVERY_URL="https://auth.example.com/application/o/tnm-hr-platform/.well-known/openid-configuration"
```
  </Step>
</Steps>
...
identity_providers:
  oidc:
    clients:
      - client_id: tnm-hr-platform
        client_name: TNM HR Platform
...
OAUTH_CLIENT_ID="tnm-hr-platform"
OAUTH_CLIENT_SECRET="your-plain-secret"
OAUTH_DISCOVERY_URL="https://auth.example.com/.well-known/openid-configuration"
```

    <Warning>
      Use the **plain text** secret in TNM HR Platform's environment, not the hashed version used in Authelia's configuration.
    </Warning>
...
<Steps>
  <Step title="Create a client">
    ...
    3. Set **Client ID** (e.g., `tnm-hr-platform`)
...
OAUTH_CLIENT_ID="tnm-hr-platform-client-id"
OAUTH_CLIENT_SECRET="tnm-hr-platform-client-secret"
OAUTH_DISCOVERY_URL="https://auth.company.com/application/o/tnm-hr-platform/.well-known/openid-configuration"
...
<CardGroup cols={2}>
  <Card title="Use HTTPS" icon="lock">
    Always use HTTPS for both your TNM HR Platform instance and OAuth provider in production. OAuth tokens should never
  ...
  <Card title="Review scopes" icon="list-check">
    Only request the scopes you need. The default (`openid profile email`) is sufficient for TNM HR Platform.
  </Card>
</CardGroup>
