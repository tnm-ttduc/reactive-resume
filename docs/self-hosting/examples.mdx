...
description: "Ready-to-use Docker Compose examples for self-hosting TNM HR Platform with Postgres, Traefik, Caddy, Nginx Proxy Manager, and other common deployment stacks."
---
...
<Info>
  **Help others by sharing your setup!** If you have a working configuration that isn't covered here, I'd love to
  include it. Simply [open a pull request](https://github.com/your-organization/tnm-hr-platform) with your example added to
...
This example uses [Traefik](https://traefik.io/) as a reverse proxy with automatic SSL certificate management via Let's Encrypt. Only the TNM HR Platform app is exposed through Traefik—Postgres remains on an internal network.
...
services:
  ...
  tnm_hr_platform:
    ...
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.tnm-hr-platform.rule=Host(`resume.${DOMAIN}`)"
      - "traefik.http.routers.tnm-hr-platform.entrypoints=websecure"
      - "traefik.http.routers.tnm-hr-platform.tls.certresolver=letsencrypt"
      - "traefik.http.services.tnm-hr-platform.loadbalancer.server.port=3000"
    healthcheck:
...
http {
    upstream tnm_hr_platform {
        server tnm_hr_platform:3000;
    }
    ...
    server {
        ...
        # Proxy settings
        location / {
            proxy_pass http://tnm_hr_platform;
...
services:
  postgres:
    ...
    networks:
      - tnm_hr_platform_network
    volumes:
      - tnm_hr_platform_postgres_data:/var/lib/postgresql
    environment:
  ...
  seaweedfs:
    ...
    networks:
      - tnm_hr_platform_network
    volumes:
      - tnm_hr_platform_seaweedfs_data:/data
    environment:
  ...
  tnm_hr_platform:
    ...
    deploy:
      ...
      labels:
        - "traefik.enable=true"
        - "traefik.http.routers.app.rule=Host(`localhost:3000`)"
        - "traefik.http.routers.app.entrypoints=websecure"
        - "traefik.http.routers.app.tls=true"
        - "traefik.http.services.app.loadbalancer.server.port=3000"

configs:
  tnm_hr_platform_postgres_config:
    name: tnm_hr_platform_postgres_config
    external: true
...
networks:
  traefik_network:
    external: true
  tnm_hr_platform_network:
    name: tnm_hr_platform_network
    driver: overlay
    attachable: true

volumes:
  tnm_hr_platform_postgres_data:
    name: tnm_hr_platform_postgres_data
  tnm_hr_platform_seaweedfs_data:
    name: tnm_hr_platform_seaweedfs_data
  tnm_hr_platform_data:
    name: tnm_hr_platform_data
```

**Deploy the stack:**

```bash
docker stack deploy -c compose-swarm.yml tnm_hr_platform
```
...
docker stack services tnm_hr_platform

# View logs for the app
docker service logs -f tnm_hr_platform_tnm_hr_platform

# Scale the app
docker service scale tnm_hr_platform_tnm_hr_platform=3

# Remove the stack
docker stack rm tnm_hr_platform
```
...
To contribute, [open a pull request](https://github.com/your-organization/tnm-hr-platform) with your example added to this page. Include:
